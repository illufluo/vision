...
void UART_Control()
{
  ...
  if (SERIAL.available())
  {
    ...
    if (inputChar == '(') { // Start loop when left bracket detected
      ...
      // If not '(', treat as command char (same as BT)
      BT_Data = inputChar;
      BT_alive_cnt = 100; // Keep alive
    }
  }
  ...
  */
  ...
  switch (BT_Data)
  {
    ...
    case 'P':  approach(); delay(1000); clip(); delay(1000); rise(); break; // Pick sequence
    case 'O':  release();                             break; // Open/Release sequence
